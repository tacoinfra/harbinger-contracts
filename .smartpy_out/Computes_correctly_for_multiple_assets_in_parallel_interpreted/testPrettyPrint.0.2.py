...
class Contract(sp.Contract):
  def __init__(self):
    self.init(assetCodes = sp.list(['BTC-USD', 'XTZ-USD']), assetMap = {'BTC-USD' : sp.record(computedPrice = 0, lastUpdateTime = sp.timestamp(0), prices = sp.record(first = 0, last = -1, saved = {0 : 0}, sum = 0), volumes = sp.record(first = 0, last = -1, saved = {0 : 0}, sum = 0)), 'XTZ-USD' : sp.record(computedPrice = 0, lastUpdateTime = sp.timestamp(0), prices = sp.record(first = 0, last = -1, saved = {0 : 0}, sum = 0), volumes = sp.record(first = 0, last = -1, saved = {0 : 0}, sum = 0))}, numDataPoints = 3, oracleContract = sp.address('KT1QLPABNCD4z1cSYVv3ntYDYgtWTed7LkYr'))
